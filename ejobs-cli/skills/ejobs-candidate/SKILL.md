...
1. **Discover before calling.** When you don't know a subcommand's parameters
   ...
   ```
   $HOME/.cache/ejobs-plugin/bin/ejobs-cli <subcommand> --describe
   ```
...
2. **Always request JSON.** Pass `--output json` on every call so your
   ...
   ```
   $HOME/.cache/ejobs-plugin/bin/ejobs-cli candidate cv get --output json
   ```
...
- **Interactive (local terminal):** ask the user to run
  `$HOME/.cache/ejobs-plugin/bin/ejobs-cli login` in their terminal. The command
...
```
$HOME/.cache/ejobs-plugin/bin/ejobs-cli candidate cv update --describe
```

### Search jobs
```
$HOME/.cache/ejobs-plugin/bin/ejobs-cli jobs list --describe
$HOME/.cache/ejobs-plugin/bin/ejobs-cli jobs list --output json --fields id,title,company,city
```
